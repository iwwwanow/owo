...
- [ ] переименовать `?static` → `/uploads/{path}` для семантики и кешируемости
  - сейчас `GET /{path}?static` используется для относительных путей внутри шаблонов — менять нужно вместе с шаблонами
  - сломает обратную совместимость; делать одним PR с миграцией всех мест использования
