...
- [ ] поддержки кириллицы. в url должен быть транслит, при этом развания папок и файлов отображатся как есть (кириллицой)
- [ ] фича `_directory-name`: создаётся разделитель, дочерние блоки рендерятся на родительской странице, в дочерней директории — `hidden`
...
- [ ] фича `_directory-name`: создаётся разделитель, дочерние блоки рендерятся на родительской странице, в дочерней директории — `hidden`
- [ ] группировка карточек по году
- [ ] разделитель с годом как label
